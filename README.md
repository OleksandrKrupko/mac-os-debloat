...
- **Works with SIP on.** Labels that launchd does not restart on demand. Any apply with SIP on installs a boot daemon (`/Library/LaunchDaemons/io.github.oleksandrkrupko.debloat.plist`) that re-disables them at every boot and at first login, then checks every minute for the first 5 minutes and every 5 minutes after that for as long as the Mac is up, killing anything that started anyway — once; a label that is running again after its kill is left alone. On macOS 26.5 with SIP on, after a reboot: `--disable-all` 170/170 overrides in effect and 168–170 of 170 stopped, `balanced` 101/101 and 100/101, `telemetry` 27/27 and 26/27. The few left running had started after the daemon's last pass, and are now marked `[sip-off]` too.
